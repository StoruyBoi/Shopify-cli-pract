...
document.querySelectorAll('[id^="Details-"] summary').forEach((summary) => {
  ...
  // Functions to open and close the submenu
  const openSubmenu = () => {
  ...
  details.addEventListener('mouseleave', () => {
    if (!isMobile()) {
      closeTimeout = setTimeout(closeSubmenu, 150); // Small delay for better UX
    }
  });

  // Add click behavior for mobile and prevent default on desktop
  summary.addEventListener('click', (event) => {
    if (isMobile()) {
      event.currentTarget.setAttribute('aria-expanded', !details.hasAttribute('open'));
    } else {
      // Prevent the click from toggling on desktop
      event.preventDefault();
    }
  });

  // Keep the escape key handler but skip for certain elements
